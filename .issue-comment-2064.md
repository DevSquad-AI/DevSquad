...
When a user runs `devsquad install` (even just to update settings), the installer:
...
- **Trigger:** Running `devsquad install` (even for unrelated updates)

### Workaround (Until Fix)

Backup your config before running install:
```bash
cp ~/.config/opencode/devsquad.jsonc ~/.config/opencode/devsquad.jsonc.backup
```
