 ...
 * @returns {string} Package name like "devsquad-darwin-arm64"
...
export function getPlatformPackage({ platform, arch, libcFamily }) {
  ...
  return `devsquad-${os}-${arch}${suffix}`;
}
 ...
 * @returns {string} Relative path like "devsquad-darwin-arm64/bin/devsquad"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/devsquad${ext}`;
}
