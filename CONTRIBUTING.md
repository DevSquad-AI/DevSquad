...
First off, thanks for taking the time to contribute! This document provides guidelines and instructions for contributing to devsquad.
...
2. **Update your OpenCode config** (`~/.config/opencode/opencode.json` or `opencode.jsonc`):
   ...
   {
     "plugin": [
       "file:///absolute/path/to/devsquad/dist/index.js"
     ]
   }
   ```
   
   For example, if your project is at `/Users/yourname/projects/devsquad`:
   ```json
   {
     "plugin": [
       "file:///Users/yourname/projects/devsquad/dist/index.js"
     ]
   }
   ```

   > **Note**: Remove `"devsquad"` from the plugin array if it exists, to avoid conflicts with the npm version.
