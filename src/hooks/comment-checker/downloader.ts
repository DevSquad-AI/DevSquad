...
/**
 * Get the cache directory for devsquad binaries.
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[devsquad] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[devsquad] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[devsquad] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[devsquad] Comment checking disabled.`)
    return null
  }
}
