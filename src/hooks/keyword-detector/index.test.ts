...
describe("keyword-detector session filtering", () => {
  ...
  test("should allow ultrawork keywords in non-main session", async () => {
    ...
    expect(toastCalls).toContain("UPUP Mode Enabled!")
  })
...
describe("keyword-detector word boundary", () => {
  ...
  test("should trigger upup on standalone 'upup' keyword", async () => {
    // given - text contains standalone 'upup'
    setMainSession(undefined)
    ...
    const output = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "upup do this task" }],
    }

    // when - message with standalone 'upup' is processed
    await hook["chat.message"](
      { sessionID: "any-session" },
      output
    )

    // then - upup should be triggered
    expect(output.message.variant).toBe("max")
    expect(toastCalls).toContain("UPUP Mode Enabled!")
  })
