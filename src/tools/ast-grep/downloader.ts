...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    log(`[devsquad] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  log(`[devsquad] Downloading ast-grep binary...`)
  ...
  try {
    ...
    log(`[devsquad] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    log(
      `[devsquad] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
