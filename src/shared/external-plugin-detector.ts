/**
 * Detects external plugins that may conflict with devsquad features.
...
/**
 * Known notification plugins that conflict with devsquad's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[devsquad] External notification plugin detected: ${pluginName}

Both devsquad and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   devsquad's session-notification has been auto-disabled.

   To use devsquad's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in devsquad.json`
}
