...
| File | Purpose |
|------|---------|
| `add-plugin-to-opencode-config.ts` | Register `devsquad` in `.opencode/opencode.json` plugin array |
...
| `generate-omo-config.ts` | Generate `devsquad.jsonc` from install selections |
...
| `plugin-name-with-version.ts` | Resolve `devsquad@X.Y.Z` for installation |
| `write-omo-config.ts` | Write generated config to `.opencode/devsquad.jsonc` |
